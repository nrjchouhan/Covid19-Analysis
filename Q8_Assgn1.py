...
# Extracting total number of dose1 and dose2 vaccinated for each district into vaccineDf DataFrame
vaccineDf = vaccine_data[['State','District_Key','District','14/08/2021.3','14/08/2021.4']]
vaccineDf = vaccineDf.rename(columns={'14/08/2021.3':'dose1','14/08/2021.4':'dose2'})
vaccineDf = vaccineDf.astype({'dose1':int, 'dose2':int})

# Merging common district keys and adding their respective dose1 and dose2 vaccinated data
...
# Merging urban and rural districts and adding their respective dose1 and dose2 vaccinated data into UrbanRuralDf DataFrame
...
# Extracting dose1 to dose2 vaccinated data for each district into vaccineDistDf DataFrame
vaccineDistDf = vaccineDf[['District_Key','dose1','dose2']]

# Extracting dose1 to dose2 vaccinated data for each state into vaccineStDf DataFrame
vaccineStDf = vaccineDf[['State','dose1','dose2']]
vaccineStDf = vaccineStDf.groupby('State').sum()
vaccineStDf = vaccineStDf.reset_index()

# Extracting dose1 to dose2 vaccinated data for overall country into vaccineOverallDf DataFrame
vaccineOverallDf = vaccineStDf.copy()
vaccineOverallDf = vaccineOverallDf.drop(['State'], axis=1)
vaccineOverallDf.insert(loc=0, column='Country', value='India')
vaccineOverallDf = vaccineOverallDf.groupby('Country').sum()
vaccineOverallDf = vaccineOverallDf.reset_index()
...
# Extracting district data from populationDf DataFrame into populationDf DataFrame which contains total population of female and male
populationDf = populationDf[(populationDf['State']>0) & (populationDf['Level']!='STATE')]
populationDf.drop('Level', axis=1, inplace=True)


# Replacing state number in 'State' column to state code in populationDf DataFrame using stateCensusDf DataFrame
...
for i in range(1,37):
    ...
    for indx in populationDf[populationDf['State'] == str(i)].index:
        populationDf.at[indx, 'State'] = st_code
populationDf = populationDf.reset_index(drop=True)

# Creating 'District_Key' column by concating state code and district name with '_' and inserting into populationDf DataFrame
populationDf['District_Key'] = populationDf['State'] + '_' + populationDf['Name']

# Making some changes in populationDistDf DataFrame as per vaccine data
populationDf['District_Key'] = populationDf['District_Key'].str.rstrip().replace('District', 'Sikkim', regex=True).replace({' +' : ' '}, regex=True)
populationDf.at[populationDf[populationDf['District_Key'] == 'MH_Raigarh'].index, 'District_Key'] = 'MH_Raigad'
...
# Updating populationDf DataFrame and correcting district names using remainingDistDict dictionary
for rd in remainingDistDict.keys():
    populationDf.at[populationDf[populationDf['District_Key'] == rd].index, 'District_Key'] = remainingDistDict[rd]
populationDf = populationDf.sort_values(by='District_Key').reset_index(drop=True)
# Adding female and male population to get total population for each district
populationDf['Total_Population'] = populationDf['TOT_F'] + populationDf['TOT_M']


# distCensusList = populationDf['District_Key'].to_list()
...
# Extracting Total Population data for each district into populationDistDf DataFrame
populationDistDf = populationDf[['District_Key','Total_Population']]

# Extracting Total Population data for each state into populationStDf DataFrame
populationStDf = populationDf[['State','Total_Population']]
populationStDf = populationStDf.groupby('State').sum()
populationStDf = populationStDf.reset_index()
for k in state_code_dict.keys():
    indx = populationStDf[populationStDf['State'] == state_code_dict[k]].index
    populationStDf.at[indx, 'State'] = k

# Extracting Total Population data for overall country into populationOverallDf DataFrame
populationOverallDf = populationStDf[['Total_Population']]
populationOverallDf.insert(loc=0, column='Country', value='India')
populationOverallDf = populationOverallDf.groupby('Country').sum()
populationOverallDf = populationOverallDf.reset_index()


# Combining vaccineDistDf and populationDistDf DataFrame into VDRatioDist DataFrame
VDRatioDist = pd.concat([vaccineDistDf.set_index('District_Key'), populationDistDf.set_index('District_Key')], axis=1).reset_index()
VDRatioDist = VDRatioDist.dropna()

# Calculating dose1 and dose2 to Total Population Ratio
VDRatioDist['vaccinateddose1ratio'] = VDRatioDist['dose1']/VDRatioDist['Total_Population']
VDRatioDist['vaccinateddose2ratio'] = VDRatioDist['dose2']/VDRatioDist['Total_Population']
VDRatioDist = VDRatioDist.rename(columns={'District_Key':'districtid'})
VDRatioDist = VDRatioDist.drop(['dose1','dose2','Total_Population'], axis=1).sort_values(by='vaccinateddose1ratio').reset_index(drop=True)


# Combining vaccineStDf and populationStDf DataFrame into VDRatioSt DataFrame
VDRatioSt = pd.concat([vaccineStDf.set_index('State'), populationStDf.set_index('State')], axis=1).reset_index()
VDRatioSt = VDRatioSt.dropna()

# Calculating dose1 and dose2 to Total Population Ratio
VDRatioSt['vaccinateddose1ratio'] = VDRatioSt['dose1']/VDRatioSt['Total_Population']
VDRatioSt['vaccinateddose2ratio'] = VDRatioSt['dose2']/VDRatioSt['Total_Population']
VDRatioSt = VDRatioSt.rename(columns={'State':'stateid'})
VDRatioSt = VDRatioSt.drop(['dose1','dose2','Total_Population'], axis=1).sort_values(by='vaccinateddose1ratio').reset_index(drop=True)


# Combining vaccineOverallDf and populationOverallDf DataFrame into VDRatioOverall DataFrame
VDRatioOverall = pd.concat([vaccineOverallDf.set_index('Country'), populationOverallDf.set_index('Country')], axis=1).reset_index()
VDRatioOverall = VDRatioOverall.dropna()

# Calculating dose1 and dose2 to Total Population Ratio
VDRatioOverall['vaccinateddose1ratio'] = VDRatioOverall['dose1']/VDRatioOverall['Total_Population']
VDRatioOverall['vaccinateddose2ratio'] = VDRatioOverall['dose2']/VDRatioOverall['Total_Population']
VDRatioOverall = VDRatioOverall.rename(columns={'Country':'overallid'})
VDRatioOverall = VDRatioOverall.drop(['dose1','dose2','Total_Population'], axis=1).sort_values(by='vaccinateddose1ratio').reset_index(drop=True)


# Generating output CSV files for every district, state and overall country from VDRatioDist, VDRatioSt and VDRatioOverall DataFrame respectively
VDRatioDist.to_csv('district-vaccinated-dose-ratio.csv', index=False)
VDRatioSt.to_csv('state-vaccinated-dose-ratio.csv', index=False)
VDRatioOverall.to_csv('overall-vaccinated-dose-ratio.csv', index=False)
